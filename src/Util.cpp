...
/***********************************************************************/
/*! @brief URLからホスト部分を抜き出す
 * http://host.co.jp　抜き出す→ "host"
 *  @param url 
 *  @retval string ホストを表す文字列
 */
/***********************************************************************/
...
/***********************************************************************/
/*! @brief 端末のMACアドレスの取得する
 * 
 *  @retval string MACアドレス
 */
/***********************************************************************/
...
/***********************************************************************/
/*! @brief delimiterで指定された文字で区切り、文字を分割する
 * 
 *  @param source 元の文字列
 *  @param delimiter 区切り文字
 *  @retval vector<string> 区切られた文字群
 */
/***********************************************************************/
...
/***********************************************************************/
/*! @brief 区切りフォーマットから時刻を得る（フォーマット例：2013_12_31_23_59_59_999）
 *
 *  @param seperated 入力文字列
 *  @retval time_t 0以上：時刻 -1:パース失敗
 */
/***********************************************************************/
...
/**
 * @brief "0x00"形式のテキストをバイナリ値（unsigned char)に変換する
 * @param x00 0x00形式のテキスト
 * @return int バイナリ値。変換に失敗した場合は-1を返す。
 */
...
/**
 * @brief 1文字、UCS-2フォーマットをUTF-8フォーマットに変換する。
 * @param ucs2 UCS-2の1文字のバイナリ値
 * @param utf8 変換後のバイナリを格納するバイナリ。4バイトの領域が必要。
 * @return int 変換後のUTF-8のバイト数。変換失敗の場合は-1が返る。
 */
...
//printf("s = %f, %f, %f\n", s0, s1, s2);
		...
		for(int j=index_start_nn;j<=index_end_nn;++j){
			...
			sum += (s2-(j-i)*s1) / (s0*s2-s1*s1) * lowess_w(abs(j-i)/(double)max_range) * *(source+j);
		}
		*(out+i) = sum;
	}
		...
		if(*(c+i) == CR){
			if(i<length-1 && *(c+i+1) == LF){
				if(index_crlf == -1){
					...
					return string(c, index_crlf, i-index_crlf);
				}
			}
		}
	}
